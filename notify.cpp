...
public:  
    ...
    void additem_workflow(const string person, const string subject, const string message)
    {
        ...
        else if(iscancellation && strip_cancellation == "Purchase-order")
        {
            ...
            removerevisemessageitem(person, revise_string, message);
            removeItemsemptymessages();
        }
    ...
    void removerevisemessageitem(const string rperson, const string rsubject, const string rmessage)
    {
        //even if different person
        //but updated subject must match, remove message that match
        noticeType *current; //pointer to traverse the list
...
int main(int argc, char* argv[])
{
    ...
    //create an array of linkedlist to hold output linkedlists for all files
    ...
    for(int i=filenumber;i<=numberoffiles; i++)
    {
        ...
        while(getline(file, line)) 
        {
            ...
            else
            {
                ...
                //process item to see if duplicate or is(cancelled) or is-cancellation 
                //or is similar to existing or is new and take appropraite action
    ...
    //cout << "lenlongest = "<< lenlongest<< "aaa"<< end;
    ...
    for (size_t i = 0; i < lenlongest; i++) // for each line of longest file
    {
        ...
        for (size_t j = 0; j < numberoffiles ; j++) //for each file
        {
            ...
            if (current[j] != NULL)
            {
                ...
                for (size_t l = 0; l <onelineofonefilemessage.size(); l++)
                {
                    //add items to global list using additem_workflow
                    globallist.additem_workflow(onelineofonefileperson, onelineofonefilesubject, onelineofonefilemessage[l]);
                }
