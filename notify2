...
struct noticeType
{
    string person;
    string subject;
    string message; // contains comma delimited messages
    noticeType *link;
};
...
public:
    ...
    //const nLinkedList& operator= (const nLinkedList&)
    //{
        
    //}

    //copy constructor
    //nLinkedList(const nLinkedList& otherList)
    //{
    //    
    //}
    //destructor
    ...
    void destroyList()
    {
        noticeType *temp; //pointer to deallocate the memoryoccupied by the node
    ...
    void print() const
    {
        ...
        {
        cout << current->person << "\t"<<current->subject << "\t" << current->message << endl;
    ...
    void additem_special(const string person, const string subject, const string message)
    {
        bool iscancellation;
        bool iscancelled;
        //cout << "inside add-item function";

        //check if subject contains is "-cancellation
        //cout << person << ":" << subject;
        if (subject.length()>13)
        {
            ...
            {
                iscancellation = true;
                //cout << " <---------is -cancellation";
            }
        }
        //check if subject contains is "(cancelled)"
        ...
        //cout <<endl;
        //if duplicates: searchduplicate on linked list  ---> ignore if exist
        if (searchDuplicateExist(person, subject, message))
        {
            //ignore;
            //cout << person << subject << " <-------- is duplicate"<<endl;
        }
/*      //elseif (cancelled): update notification_item.subject - remove "(cancelled)" then searchduplicate ---> remove existing notice
        else if(iscancelled)
        {
            //search for s
            string strip_cancelled = subject.substr(0,subject.length()-11);
            if (searchDuplicateExist( person, strip_cancelled, message))
            {
                //add code to remove the existing nb (take care) of situation with other bodies have been appended
            }
    
        }
        //elseif -cancellation and -revise: update notification_item.subject - remove "-cancellation" or "-revise" then findMatchingSubjectBody ---> remove existing, addnew
  */
         else if(iscancellation)
        {
            //search for s
            string strip_cancellation = subject.substr(0,subject.length()-13);
            if (searchSubjectMessageExists(person,  strip_cancellation, message))
            {
                //add code to remove the existing of situation with other bodies
                //removeitemsubjectmessage()
            }
            string revised = strip_cancellation + "-revised";
            if (searchSubjectMessageExists(person,  revised, message))
            {
                //add code to remove any revised (take care) of situation with other bodies
                //removeitemsubjectmessage()
            }
        }
    ...
    void addNewItemtoback(const string nperson, const string nsubject, const string nmessage)
    {
        ...
        newNode->message = nmessage; //store the new item in the node
    ...
    bool searchDuplicateExist(const string sperson, const string ssubject, const string smessage)
    {
        ...
        while (current != NULL && !found)
        {
            string tempPerson = current->person;
            string tempSubject = current->subject;
            string tempMessage = current->message;
            if ( tempPerson == sperson && tempSubject == ssubject && tempMessage.find(smessage) != string::npos) //searchItem is found
                found = true;
            else
                current = current->link; //make current point tothe next node
        }
    ...
    bool searchSubjectMessageExists(const string sperson, const string ssubject, const string smessage)
    {
        ...
        { 
            string tempSubject = current->subject;
            string tempMessage = current->message;
            if (tempSubject == ssubject && tempMessage.find(smessage) != string::npos) //searchItem is found
                found = true;
            else
                current = current->link; //make current point tothe next node
        }
    ...
    void removeItemsubjectmessage(const string sperson, const string ssubject, const string smessage)
    {
        ...
        while (current != NULL && !found)
        {
            ...
            string tempSubject = current->subject;
            string tempMessage = current->message;
            if ( tempSubject == ssubject && tempMessage.find(smessage) != string::npos) 
            {
                //searchItem is found
                //1. updatedmessage = removefromcommaspacedelimitedstring(const string tempMessage, const string smessage)
...
int getnumberoffiles( string inputfile)
{
    ...
    //cout << "inputfile 0 = " << inputfile << endl;
    string input_file_prefix = inputfile.substr(0,inputfile.find_first_of('.')+1);
    //cout << "first inputfile prefix = " << input_file_prefix << endl;
    int input_file_number = stoi( inputfile.substr(inputfile.find_first_of('.')+1,1));
    //cout << "first inputfile number = " << input_file_number << endl;

    bool more_files_to_read = true;

    //getting  the count of files that will be read
    int numberoffiles = 0;
    ...
    while (more_files_to_read)
    {
        ...
        if (file) 
        {
            ...
            //cout << "currently processing filename: " << inputfile<<endl;
            numberoffiles++;
            input_file_number++;
        ...
        else
        {
            ...
            //cout << "no more inputfiles to read" << endl;
...
int main(int argc, char* argv[])
{
    ...
    for(int i=0;i<numberoffiles; i++)
    {
        ...
        string filetoopen =  fileprefix+to_string(i+1)+".txt";
